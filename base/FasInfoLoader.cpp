...
fas::FasInfoLoader::FasInfoLoader(const char* fname)
{
    _tm = 0;
    _using_no = 1;
    _fname.clear();
...
fas::FasInfoLoader::~FasInfoLoader()
{
...
int fas::FasInfoLoader::need_reload() const
{
...
int fas::FasInfoLoader::load()
{
    ...
    FasInfo* p_t = new ( std::nothrow ) FasInfo();
    if (p_t == NULL) {
        return -1;
    }
...
int fas::FasInfoLoader::reload()
{
    ...
    fas::FasInfo* p_t = new ( std::nothrow ) FasInfo();
    if (p_t == NULL) {
        return -1;
    }
...
fas::FasInfo* fas::FasInfoLoader::get_content()
{
    //这段代码有并发问题
...
const fas::FasInfo* fas::FasInfoLoader::get_content() const
{    
    return get_content();
}
